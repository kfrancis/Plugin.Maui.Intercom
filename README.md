...
| | Version |
|----------|---------------------------|
| .NET | **.NET 9 and .NET 10** (`net9.0`, `net10.0`, plus Android/iOS platform assets) |
...
### Shared view-model libraries

The package also ships plain `net9.0` and `net10.0` assets. Reference it from an application-core library and depend on `IIntercom`; do not call `Intercom.Default` there, because its plain-.NET implementation intentionally throws. The MAUI head calls `UseIntercom()` and registers the Android/iOS implementation, which constructor injection then supplies to your view models.

```csharp
public sealed class SupportViewModel(IIntercom intercom)
{
    public void OpenMessenger() => intercom.Present();
}
```
...
The main package declares the binding packages as platform-conditional dependencies (any `-ios` target framework → iOS binding, any `-android` → Android binding), so consumers only ever add `Plugin.Maui.Intercom`. It carries plain `net9.0`/`net10.0` contract assets for shared libraries and more-specific `net9.0-*`/`net10.0-*` assets for the MAUI heads.
...
Runs on plain `net10.0` against the package project's generic target. It covers the model invariants, the generic-.NET fallback (every `IIntercom` member must be present and must throw), and the integrity of `eng/api-coverage.json` — every `covered` entry has to name a member that really exists, which is the half `eng/api-coverage.sh` cannot check.
